 ...
 */
export type ParsedFilterColumnTypes = Readonly<
  Record<string, AvaDataType.T | undefined>
>;
  ...
  if (node.type === "group") {
    return {
      ...
      rules: node.rules.map((child) => {
        return stampFilterColumnTypes(child, columnTypes);
      }),
    };
  }
...
export function parseWhereNode(
  ...
  if (operator === "AND" || operator === "OR") {
    const left = parseWhereNode(obj.left, unmappedReasons, columnTypes);
    const right = parseWhereNode(obj.right, unmappedReasons, columnTypes);

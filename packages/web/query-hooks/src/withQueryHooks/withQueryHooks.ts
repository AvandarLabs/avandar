    ...
    .forEach((mutationFnName) => {
      ...
      if (typeof clientFunction === "function") {
        ...
        const useClientMutation = (
          ...
          // Disables `react-doctor/query-mutation-missing-invalidation`.
          //
          // This is our own `useMutation` wrapper, not Tanstack's. It performs
          // invalidation from the `queriesToInvalidate` option passed below
          // (built just above from `queryToInvalidate`, `queriesToInvalidate`,
          // and `invalidateGetAllQuery`), rather than from an `onSuccess`
          // calling `queryClient.invalidateQueries`. That rule only recognizes
          // the latter shape, so it reports a false positive here.
          //
          // `react-doctor-disable-next-line` takes no rule name, so this
          // suppresses every rule on the next line, not just that one. Keep the
          // line it guards to the single `useMutation(` call.

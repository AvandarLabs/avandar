...
- **Depends on**:
  - ~~`078-lingui-scaffold`~~ **— resolved 2026-06-10.** PR #242 squash-merged Lingui onto develop at `2881b0bb`. `@lingui/*` v6.2.0 packages and the `i18n:*` scripts are present in `develop`'s `package.json`.
...
- **2026-06-10 — migration attempted, abandoned.** Operator started `/deslop migrate async-dataset-import-pipeline`. Worktree `refactor-001/async-dataset-import-pipeline` was created off `2881b0bb`, all 40 listed paths were ported byte-clean from `feat/ict4d-demo`, `pnpm install` succeeded — then `pnpm tsc -b --noEmit` produced ~40+ errors traceable to cross-feature imports the plan did not list. **Five inventory rows must land before this one can compile:** `#061 web-offline-mode`, `#077 analytics-client-events`, `#094 chat-models-catalog-regeneration`, and the `#083`-`#086` PTRCK billing series (for `SubscriptionModule` + the renamed `SubscriptionRead.id`). Worktree + branch were cleaned up; row stays `[ ]` in `ALL_FEATURES.md`. **Sequencing implication:** do NOT pick this row off `/deslop continue` until those rows are merged. The next undrift pass should re-check each dep before greenlighting another attempt.
- **2026-06-10 — DuckDbClient scope expansion required.** The ported `LocalDatasetClient.ts` calls `duckdb.sniffCsv(...)` and reads `parquetData` off `DuckDbLoadCsvResult` / `DuckDbLoadXlsxResult` — neither exists on develop's `src/clients/DuckDbClient/`. Diff vs develop on `src/clients/DuckDbClient/` is **12 files, +1819 / −322 lines** (including a new `csvParse/` subdir with `csvParseOptions.ts`, `csvQuoteProbe.ts`, plus `duckDbManualBundles`, `shouldLoadDuckDbNetworkExtensions`). These are part of THIS row's surface — the parquet streaming pipeline and the CSV sniffing both live in DuckDbClient. When this migration is re-attempted, **add `src/clients/DuckDbClient/**` to the Files-to-edit list.**
- **2026-06-10 undrift — Lingui blocker resolved.** Row #078 (`lingui-scaffold`) landed on develop as PR #242 (`2881b0bb`). `@lingui/*` v6.2.0 deps and the `i18n:*` scripts are in develop's `package.json`. TSX files in this row can keep their `@lingui/react/macro` imports verbatim during the port. **Cross-cutting impact:** this same blocker affected most other UI rows on the board; all of them are now unblocked.

...
- **Depends on**: ~~`078-lingui-scaffold`~~ **— resolved 2026-06-10.** PR #242 squash-merged Lingui onto develop at `2881b0bb`. `@lingui/*` v6.2.0 packages and the `i18n:*` scripts are present in `develop`'s `package.json`. The TSX UI files in this row's scope (`DataImportView.tsx`, `DataImportTabs.tsx`, `ManualUploadView.tsx`, `DatasetImportForm.tsx`, `GoogleSheetsImportView.tsx`, `ResyncDatasetCard.tsx`, `DatasetNavbar.tsx`) can now be ported with their `@lingui/react/macro` imports intact.

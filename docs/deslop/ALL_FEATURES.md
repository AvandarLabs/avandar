...
5. `GROUP-5-platform-i18n-standalone.md` — #056–#063, #079–#082, #090, #095, #091–#093, **+ parity-absorption #098–#101 (added 2026-08-03; see §S)**
...
| 48 | `[x] (4f57526a)` | **sql-form-sync-dashboards** — Per-block `useDashboardManualQueryState` hook giving DataViz blocks the same SQL ↔ form parity as Data Explorer; 3-tab `NLQueryPField` (Prompt / Manual / SQL). | CHECKPOINT 16 |
...
| # | Status | Feature | Sources |
|---|---|---|---|
| 64 | `[x] (4f57526a)` | **dashboard-design-tokens** — `AvaPageRootProps` gains `theme` (default/ocean/forest/rose/amber/graphite) and `typography` (system/serif/mono); polished header (left-accent strip, tighter title leading, uppercase byline); polished DataViz card. | CHECKPOINT 9 (dashboards) |
| 65 | `[x] (4f57526a)` | **dashboard-chat-in-editor** — `addDashboardBlock` tool; `DashboardEditorStateManager` queues blocks; chat composer unlocked on dashboards surface; `buildPendingDataVizBlock`. | CHECKPOINT 9 (dashboards) |
| 66 | `[x] (4f57526a)` | **dashboard-export-buttons-polish** — Updated dashboard buttons and export UI styling. | Commits `9d4ac78`, `7abad7d` |
| 67 | `[x] (4f57526a)` | **dashboard-modal-styles** — Modal style refresh used by dashboards (Publish, Export, etc.). | Commit `98dc225`, `5eed96a` |

## J. Dashboards — filters

| # | Status | Feature | Sources |
|---|---|---|---|
| 68 | `[x] (4f57526a)` | **dashboard-global-filters** — `Filter` P-block (single-select, multi-select, contains); `DashboardFilterStateManager`; `applyDashboardFiltersToSql` subselect wrap. | CHECKPOINT 9 (dashboards) |
| 69 | `[x] (4f57526a)` | **dashboard-per-viz-filters** — Per-viz global-filter All/Some/None opt-out; per-viz local filters with independent state; `AvaPageDataMigrationV4` seeds defaults on every DataViz block. | CHECKPOINT 14 |

## K. Dashboards — publishing & sharing

| # | Status | Feature | Sources |
|---|---|---|---|
| 70 | `[x] (4f57526a)` | **dashboard-view-before-publish** — Auth-gated preview route `/<workspaceSlug>/dashboards/preview/<dashboardId>` with "Back to editor" banner; `mode: "public" \| "preview"` prop on `DashboardViewerView`. | CHECKPOINT 6 |
| 71 | `[x] (4f57526a)` | **dashboard-publish-modal** — Real modal replaces confirm dialog. URL-first copy ("Your dashboard will be published to: <url>"). | CHECKPOINT 6 + CHECKPOINT 8 polish |
| 72 | `[x] (4f57526a)` | **dashboard-vanity-url** — Kebab-case slug input + live preview; `toVanitySlug` utility with 8 unit tests; public route `/d/<workspaceSlug>/<slug>` with workspace-scoped uniqueness. | CHECKPOINT 6 + 8 |
| 73 | `[x] (4f57526a)` | **dashboard-share-url-row-qr** — `ShareUrlRow` shows canonical + vanity URLs with copy buttons; downloadable 256×256 QR PNG via `qrcode` library (client-side, no network). | CHECKPOINT 6 |
| 74 | `[x] (4f57526a)` | **dashboard-slice-aware-publish** — `Data scope` section with `queried` (default, narrowest) / `all_columns` / `custom` modes; `node-sql-parser` `columnList` extracts referenced columns; `unparseable` sentinel for safe fallback; `buildSliceSql` materializes the slice; persists in `dashboard.config.__publishConfig`. | CHECKPOINT 13 |
| 75 | `[x] (4f57526a)` | **dashboard-pdf-export-annotate** — `ExportPdfButton` next to Publish; two-step modal (export immediately / annotate then export); off-screen render via `<PuckPageRender>` + `html2canvas` 2× → `jspdf` paginated portrait letter; annotator with freehand/arrow/text (RoughJS), roughness/stroke/color sliders, undo+clear, composited before pagination. Currently gated behind a local `HIDE_EXPORT_AS_PDF = true` flag — migrate the gate alongside the feature so it ships defaulted-off. | CHECKPOINT 13; commit `6fee1d3d` (HIDE_EXPORT_AS_PDF flag) |

## L. Dataset summary view

| # | Status | Feature | Sources |
|---|---|---|---|
| 76 | `[x] (4f57526a)` | **summary-view-redesign** — `DatasetSummaryView` doc-style outline with sticky TOC, one section per column with plain-language headline + type-appropriate viz (text/number/date), missing-rate `RingProgress` when nonzero, lazy `getColumnSummary` via `useIntersection` 200px margin. New `getDatasetMeta`/`getColumnSummary` on `DatasetQueryClient`. | CHECKPOINT 6 |
...
## S. Earlier-group residuals — parity absorption (added 2026-08-03)

Surfaced by the 2026-08-03 final-parity audit (`git diff origin/develop..origin/feat/ict4d-demo` over `src shared packages supabase scripts` = 362 files / 158 feat-only, after the G4 mergeback). These are **feat-ahead subsystems and cleanup that earlier groups' mergebacks deferred** — not new commits, so `/deslop update` cannot see them. They are folded into GROUP-5 so that cutting it truly reaches parity (see `GROUP-5-*.md` → "Residual parity audit"). The bulk of the remaining drift (~285 files) is `#081` i18n-wiring (string wrapping + static-label-fn → localized-hook conversions across every surface) and is already covered by that row.

| # | Status | Feature | Sources |
|---|---|---|---|
| 98 | `[~]` | **chat-privacy-runtime-integration** — Thread the **already-on-develop** privacy detectors into the live chat turn. **Correction (2026-08-03):** G3 DID migrate the whole privacy subsystem to develop — detectors at `src/components/privacy/privacy-helpers/{detectPii,detectBias,decideIfDataCanCrossBoundary,discoveryQuery,generatedSqlAssumptions,patterns/*,SessionSecret,PendingAcks}`, `ConsentModal/*`, `src/models/privacy/*`, `src/clients/privacy/*`, `PrivacyLogTab/*`, `supabase/functions/_shared/privacy/*`. The real residual is that develop's `useAvandarChatRuntime.ts` (102 lines) **never invokes** them (dead detectors), while feat's (498 lines) wires `crossBoundary`/consent/`detectBias` gating into every turn — the integration G3's mergeback explicitly deferred "in coordination with G4/G5". Work: port feat's turn wiring in `useAvandarChatRuntime.ts` (+ `ClarificationCard/clarificationAnswer`, `PostChatMessages` consent path), repointing imports from feat's stale `src/lib/privacy/` to develop's `src/components/privacy/privacy-helpers/`, then **delete feat's stale `src/lib/privacy/**` duplicate** (13 files; only live importer is the runtime). | G3 mergeback deferred runtime wiring (2026-07-31 STATE log) |
| 99 | `[~]` | **sql-display-stale-duplicate-cleanup** — **NOT a migration (correction 2026-08-03).** The SQL display/mention subsystem is already on develop at `src/components/sql/sql-helpers/{formatSqlForDisplay,buildSqlDisplaySegments,createSqlMentionExtension,computeSqlScope,buildSqlDisplayCatalog}` (via G2/G3). Feat carries a **dead stale duplicate** at `src/lib/sql/*` (8) + `shared/lib/sql/*` (5) with **no live importers** — the mergeback didn't delete the old-path copies. Work: delete both stale dirs on feat; parity restored. | G2/G3 mergeback residual (stale duplicate) |
| 100 | `[~]` | **session-expired-recovery** — Genuinely feat-ahead (verified absent on develop): `packages/shared/clients/src/ServerApiClient/*` (`ServerApiSessionRefresher`, `createBrowserServerApiClient`, `createIpcServerApiClient`, `createServerApiClient`), `src/components/AppErrorBoundary/*` (recovery/generic-error/manual-recovery/session-recovering screens + `isSessionError` + `recoverFromSessionError`), `src/config/registerSessionExpiredHandler.ts`, `src/clients/APIClient.ts`, `supabase/functions/_shared/MiniServer/*`. develop has partial `ServerApiClient` scaffolding + shares `notifyExpiredSession.ts`; feat is ahead with the full refresh/recovery flow. ~15 files. | feat `ServerApiClient`, `AppErrorBoundary`, `registerSessionExpiredHandler` |
| 101 | `[~]` | **earlier-group-rename-and-schema-residuals** — Reconcile at the final-parity pass. (a) **Adopt-develop's-rename cleanups** (develop has the clean version, feat carries a stale duplicate — same class as #099): `analyticsClient.ts` → develop's `AnalyticsClient.ts` (repoint feat's `logAnalyticsEvent` call sites to `AnalyticsClient.logEvent`); `structuredQueryToSQL.ts` → develop's `structuredQueryToSql`; `ChartStyle.ts`. (b) **Feat-only G1 leftovers** develop never received: `src/views/DataExplorerApp/OpenDatasetDrawer/datasetPreviewSQL.ts`, `src/views/DataManagerApp/DataImportView/useCanAddDataset.ts` (both named in the G1 mergeback log). (c) **Schema**: feat-only migration `supabase/migrations/20260727210438_remove_chat_planning.sql` + `supabase/tests/database/datasets__virtual_contract.test.sql` — a Phase-1 schema gap to reconcile onto develop. | G1/G2 mergeback residuals; feat-only migration |

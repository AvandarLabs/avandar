...
| # | Status | Feature | Sources |
|---|---|---|---|
| 98 | `[ ]` | **chat-privacy-clarification-runtime** — Thread the privacy/clarification subsystem into the live chat turn lifecycle. G3 ported the privacy *clients/components* to develop but **deferred the runtime wiring** (its mergeback log: "ported + unit-tested but not yet threaded into the live turn lifecycle… complete in coordination with G4/G5"). Feat-only and absent on develop: the whole `src/lib/privacy/**` (piiDetector, biasDetector, crossBoundary, consentAuditLog, clarificationAuditLog, discoveryQuery, sessionSecret, pendingAcks, generatedSqlAssumptions, isRowDataMessage, ackToken + `patterns/{es,fr}/biasPatterns`, ~20 files), `supabase/functions/_shared/privacy/{ackToken,discoveryQuery}.ts`, `supabase/functions/chat/dataExplorerToolDefinitions/*`, and the feat runtime wiring in `src/components/ChatPanel/{useAvandarChatRuntime.ts,ClarificationCard/clarificationAnswer.ts,ChatEmptyState/pickChatSuggestionColumns.ts}`. | G3 mergeback residual (2026-07-31 STATE log); feat `src/lib/privacy/**` |
| 99 | `[ ]` | **sql-display-mention-rendering** — Feat-only SQL display/mention rendering subsystem (SQL pills, `@mention` autocomplete, highlight segments) absent on develop: `shared/lib/sql/{buildSqlDisplaySegments,sqlScope,sqlDisplay.types}.ts` and `src/lib/sql/{formatSqlForDisplay,buildSqlDisplayCatalog,createSqlDisplayExtension,createSqlMentionExtension,getSqlMentionOptions}.ts` (+ tests) — 13 files. | feat `src/lib/sql/*`, `shared/lib/sql/*` |
| 100 | `[ ]` | **session-expired-recovery** — Feat-ahead session-expiry handling: `packages/shared/clients/src/ServerApiClient/*` (`ServerApiSessionRefresher`, `createBrowserServerApiClient`, `createIpcServerApiClient`, `createServerApiClient`), `src/components/AppErrorBoundary/*` (recovery/generic-error/manual-recovery/session-recovering screens + `isSessionError` + `recoverFromSessionError`), `src/config/registerSessionExpiredHandler.ts`, `src/clients/APIClient.ts`, `packages/web/ui/src/notifications/notifyExpiredSession.ts`, and `supabase/functions/_shared/MiniServer/*`. develop has partial `ServerApiClient` scaffolding; feat is ahead with the full refresh/recovery flow. ~17 files. | feat `ServerApiClient`, `AppErrorBoundary`, `registerSessionExpiredHandler` |
| 101 | `[ ]` | **earlier-group-rename-and-schema-residuals** — Small G1/G2 leftovers to reconcile at the final-parity pass. (a) **Adopt-develop's-rename cleanups** (develop has the clean version, feat carries a stale duplicate): `analyticsClient.ts` → develop's `AnalyticsClient.ts` (repoint feat's `logAnalyticsEvent` call sites to `AnalyticsClient.logEvent`); `structuredQueryToSQL.ts` → develop's `structuredQueryToSql`; `ChartStyle.ts`. (b) **Feat-only G1 leftovers** develop never received: `src/views/DataExplorerApp/OpenDatasetDrawer/datasetPreviewSQL.ts`, `src/views/DataManagerApp/DataImportView/useCanAddDataset.ts` (both named in the G1 mergeback log). (c) **Schema**: feat-only migration `supabase/migrations/20260727210438_remove_chat_planning.sql` + `supabase/tests/database/datasets__virtual_contract.test.sql` — a Phase-1 schema gap to reconcile onto develop. | G1/G2 mergeback residuals; feat-only migration |

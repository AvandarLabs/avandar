...
| 61 | `[ ]` | **web-offline-mode** — PWA + service worker, React Query persistence (`avandar-react-query-cache`), UI gates / disabled controls when offline, offline banners. **Required by #001** — ported TSX files import from `@/lib/offline/useOfflineGate`, `@/lib/offline/useIsOnline`, `@/lib/offline/useLocalDatasetIds`, `@/components/offline/OfflineGated`, `@/components/offline/OfflineUnavailableTooltipLabel`. | `docs/demo-features/web-offline-mode.md`; `docs/superpowers/plans/2026-05-20-web-read-only-offline-mode-demo.md`; commits `c597869`, `7740537`, `207d422` |

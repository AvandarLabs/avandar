...
- **`uuid()` needs its brand.** Bare `uuid()` returns `UUID<never>`, which does not satisfy a branded id such as `DatasetColumnId`. Always pass the type argument: `uuid<DatasetColumnId>()`.

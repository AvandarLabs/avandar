...
Playwright drives Chromium beautifully. Playwright cannot drive WKWebView. The React app inside our Electrobun shell _is_ the same code that runs in the web browser — but it branches on `window.__AVA_PLATFORM__` to pick desktop adapters that call `window.electrobun.callIpc(...)` to reach Bun-main services.
...
export const test = base.extend<{ harness: Harness }>({
  harness: async ({ page }, use) => {
    ...
    await page.exposeFunction("__avaCallIpc", (channel: string, msg: unknown) =>
      ipcServer.dispatch(channel, msg),
    );

...
| Tutorial definition | Four milestones and ten tooltips, declared as pure data |
...
is what took the tutorial from fifteen tooltips down to ten. Arrival tooltips
...
| 3 | `build_dashboard` | Build your first dashboard | 2 | dashboard created |
| 4 | `share_dashboard` | Share it with your workspace | 3 | general access set to `workspace` |

Ten tooltips in chunks of 3/2/2/3. Two-tooltip milestones matter
psychologically: the end is visible from the start.
...
| # | Anchor | Draft copy |
| --- | --- | --- |
| 1 | `chat-composer` | This is the Data Explorer, and this is Ava. Ask a question in plain English, like "total revenue by region". |
...
| # | Anchor | Draft copy |
| --- | --- | --- |
| 1 | `explorer-viz-tab` | Want a different shape? The Visualizations tab has the chart settings. |
| 2 | `explorer-save-menu` | Now open Save and choose "Save to dashboard". Name the dashboard, create it, and Avandar will take you there. |

Tooltip 2 anchors the **Save menu's trigger button**, not the "Save to
dashboard" item inside it. The item lives in a Mantine `Menu.Dropdown` that
closes on any outside click, including a click on Joyride's overlay, so
spotlighting the item directly would make the step unusable. The copy names
the item instead, and also covers the create-mode modal rather than spending a
third tooltip on a form that explains itself.
...
distinguishes "the dataset *you just made*" from "a dataset exists", it fires
...
`public/samples/avandar-sample-sales.csv`, roughly 200 rows:

| Column | Type | Why it is there |
| --- | --- | --- |
| `order_date` | date | so the summary view shows its date timeline |
| `region` | text, 4 values | a text frequency bar, and milestone 2's group-by |
| `product_category` | text, 5 values | a second frequency bar, a second grouping option |
| `units_sold` | integer | a numeric stat block |
| `revenue_usd` | numeric | the obvious thing to measure in milestone 2 |

Chosen so milestone 1's payoff tooltip has all three summary visuals to point
at, and so "revenue by region" is the obvious first question in milestone 2.
...
| Mantine `Menu.Dropdown` closes on any outside click, including Joyride's overlay | No step ever targets an element inside an open menu. Milestone 3 anchors the menu's trigger and names the item in copy. |

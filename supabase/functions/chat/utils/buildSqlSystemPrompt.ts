...
/**
 * Best-effort extractor for SQL embedded in free-form assistant text.
 *
 * Some models forget to invoke the `generateSql` tool and just spit a
 * SELECT (or a `\`\`\`sql\` fenced block) in their chat message. When the
 * tool-call path produced nothing, the chat handler falls back to this so
 * the canvas still gets to run a query.
 *
...
export function extractSqlFromAssistantText(text: string): string | undefined {
  ...
  const fenced = text.match(FENCED_SQL_REGEX);
  if (fenced && fenced[1]) {
    const cleaned = cleanGeneratedSql(fenced[1]).replace(/;\s*$/, "").trim();
    if (cleaned.length > 0) {
      return cleaned;
    }
  }
  const bare = text.match(BARE_SQL_REGEX);
  if (bare && bare[0]) {
    const cleaned = cleanGeneratedSql(bare[0]).replace(/;\s*$/, "").trim();
    // Guard against false positives like "Select the dataset you want".
    if (cleaned.length > 0 && /\b(FROM|WITH)\b/i.test(cleaned)) {
      return cleaned;
    }
  }
  return undefined;

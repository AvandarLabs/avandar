 ...
 * views.
 *
 * `other` is the fallback for an event name that
 * `public.util__analytics_event_category` has not categorised, so a typo'd or
 * newly-added name can never reject an insert. Find them with

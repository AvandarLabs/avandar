...
create table public.entity_field_configs (
  ...
  entity_config_id uuid not null references entity_configs (id) on update cascade on delete cascade,
  name text not null,
  description text,
...
" on public.entity_field_configs for
select
  to authenticated using (
    public.entity_field_configs.workspace_id = any (
      array(
        select
...
" on public.entity_field_configs for insert to authenticated
with
  check (
    public.entity_field_configs.workspace_id = any (
      array(
...
" on public.entity_field_configs for delete to authenticated using (
  public.entity_field_configs.workspace_id = any (
    array(
      select
        public.util__get_auth_user_workspaces ()
 ...
 */
create trigger tr_entity_field_config__set_updated_at before
update on public.entity_field_configs for each row
execute function public.util__set_updated_at ();

/**
 * Validate title and id fields.
 ...
 */
create or replace function public.entity_field_configs__validate_title_and_id_fields () returns trigger as $$
begin
...
/**
 * Triggers the title and id validations for the fields and extractors.
 *
 ...
 * we need to manually rollback the changes. Currently the rollback needs

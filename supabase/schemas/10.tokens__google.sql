...
" on public.tokens__google for
select
  to authenticated using (
    public.tokens__google.user_id = (
      select
        auth.uid ()
    )
  );
...
" on public.tokens__google for insert to authenticated
with
  check (
    public.tokens__google.user_id = (
      select
...
" on public.tokens__google for delete to authenticated using (
  public.tokens__google.user_id = (
    select
      auth.uid ()
  )

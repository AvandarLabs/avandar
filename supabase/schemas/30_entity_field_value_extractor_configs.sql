...
-- Create the value_extractor__dataset_column_value table
create table public.value_extractor__dataset_column_value (
    ...
    entity_field_config_id uuid not null references entity_field_configs(id)
        on update cascade
        on delete cascade,
    value_picker_rule_type public.value_extractor__value_picker_rule_type not null,
    dataset_id uuid not null,
    dataset_field_id uuid not null
);

-- Create the value_extractor__manual_entry table
...
-- Create the value_extractor__aggregation table
create table public.value_extractor__aggregation (
    ...
    entity_field_config_id uuid not null references entity_field_configs(id)
        on update cascade
        on delete cascade,
    aggregation_type public.value_extractor__aggregation_type not null,
    dataset_id uuid not null,
    dataset_field_id uuid not null,
    filter jsonb
);
...
create policy "User can SELECT value_extractor__dataset_column_value"
    on public.value_extractor__dataset_column_value for select
    to authenticated -- postgres role
    -- actual policy
    using (true);

create policy "User can SELECT value_extractor__manual_entry"
    on public.value_extractor__manual_entry for select
    to authenticated -- postgres role
    -- actual policy
    using (true);

create policy "User can SELECT value_extractor__aggregation"
    on public.value_extractor__aggregation for select
    to authenticated -- postgres role
    -- actual policy
    using (true);
...
create policy "User can INSERT value_extractor__dataset_column_value"
    on public.value_extractor__dataset_column_value for insert
    to authenticated -- postgres role
    -- actual policy
    with check (true);

create policy "User can INSERT value_extractor__manual_entry"
    on public.value_extractor__manual_entry for insert
    to authenticated -- postgres role
    -- actual policy
    with check (true);

create policy "User can INSERT value_extractor__aggregation"
    on public.value_extractor__aggregation for insert
    to authenticated -- postgres role
    -- actual policy
    with check (true);
...
create policy "User can UPDATE value_extractor__dataset_column_value"
    on public.value_extractor__dataset_column_value for update
    to authenticated -- postgres role
    -- actual policy
    with check (true);

create policy "User can UPDATE value_extractor__manual_entry"
    on public.value_extractor__manual_entry for update
    to authenticated -- postgres role
    -- actual policy
    with check (true);

create policy "User can UPDATE value_extractor__aggregation"
    on public.value_extractor__aggregation for update
    to authenticated -- postgres role
    -- actual policy
    with check (true);
...
create policy "User can DELETE value_extractor__dataset_column_value"
    on public.value_extractor__dataset_column_value for delete
    to authenticated -- postgres role
    -- actual policy
    using (true);

create policy "User can DELETE value_extractor__manual_entry"
    on public.value_extractor__manual_entry for delete
    to authenticated -- postgres role
    -- actual policy
    using (true);

create policy "User can DELETE value_extractor__aggregation"
    on public.value_extractor__aggregation for delete
    to authenticated -- postgres role
    -- actual policy
    using(true);

-- Create updated_at triggers
create trigger tr_value_extractor__dataset_column_value_set_updated_at
    before update on public.value_extractor__dataset_column_value
    for each row
    execute function public.util__set_updated_at();

create trigger tr_value_extractor__manual_entry_set_updated_at
    before update on public.value_extractor__manual_entry
    for each row
    execute function public.util__set_updated_at();

create trigger tr_value_extractor__aggregation_set_updated_at
    before update on public.value_extractor__aggregation
    for each row
    execute function public.util__set_updated_at();
...
-- Data API privileges.
--
-- No `delete` for `authenticated`: restarting the tutorial updates the row
-- in place, so there is no delete policy and nothing to grant. Nothing for
-- `anon` either, because an unauthenticated visitor has no onboarding
-- progress.
grant
select
,
...
,
  insert,
update,
delete on table public.user_nux_progress to service_role;

 ...
 * Enum values:
 *
 * `publish` - Staging a new snapshot generation for the audience named by
 *  `snapshot_transition_target_visibility`.
 *
 * `abort_publish` - Fences an abandoned `publish` so its staged generation
 *  can be cleaned up, restoring the prior visibility and revision.
 *
 * `unpublish` - Withdrawing a published dashboard.
 *
 * `delete` - Removing the snapshot objects and then the row. Not terminal: it
 *  settles like `unpublish`, so an abandoned delete leaves a usable draft
 *  rather than an unrepairable row.
 *
 * Which values may be claimed from idle, and which only as a fence over an
 * existing claim, is defined by `private.dashboards__snapshot_claim_is_valid`
 * and `private.dashboards__snapshot_progress_is_valid`.
 *

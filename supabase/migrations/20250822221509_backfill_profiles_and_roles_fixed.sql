...
select gen_random_uuid(), wm.id, wm.user_id, wm.workspace_id,
...
select gen_random_uuid(), wm.workspace_id, wm.id, 'member', now(), now()
from public.workspace_memberships wm
left join public.user_roles r
  on r.membership_id = wm.id and r.role = 'member'

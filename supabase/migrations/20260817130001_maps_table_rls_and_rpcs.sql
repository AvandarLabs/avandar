...
create unique index maps__slug_unique_when_public on public.maps using btree (slug)
where
  (
    (is_public = true) and
    (slug is not null)
...
add constraint "maps_pkey" primary key using index "maps_pkey";

alter table "public"."maps"
add constraint "maps_owner_id_fkey" foreign key (owner_id) references auth.users (id) on update cascade not valid;

alter table "public"."maps" validate constraint "maps_owner_id_fkey";

alter table "public"."maps"
add constraint "maps_owner_profile_id_fkey" foreign key (owner_profile_id) references public.user_profiles (id) on update cascade not valid;

alter table "public"."maps" validate constraint "maps_owner_profile_id_fkey";

alter table "public"."maps"
add constraint "maps_workspace_id_fkey" foreign key (workspace_id) references public.workspaces (id) on update cascade on delete cascade not valid;
...
create or replace function public.maps__auth_user_may_select (p_map_id uuid) returns boolean language plpgsql stable security definer
set
...
create or replace function public.maps__owner_id_matches_stored (p_map_id uuid, p_owner_id uuid) returns boolean language sql stable security definer
set
  search_path to '' as $function$
  select
...
grant
select
  on table "public"."maps" to "authenticated";

grant
update on table "public"."maps" to "authenticated";

create policy "Users can read maps they have permissions for" on "public"."maps" as permissive for
select
  to authenticated using (
    (
      ...
      public.maps__auth_user_may_select (id)
    )
  );

create policy "Users with admin access can delete maps" on "public"."maps" as permissive for delete to authenticated using (
  public.util__auth_user_can_delete_resource ('map'::public.resource_type, id)
);

create policy "Users with editor access can update maps" on "public"."maps" as permissive
for update
  to authenticated using (
    public.util__auth_user_can_update_resource ('map'::public.resource_type, id)
  )
with
  check (
    (
      public.util__auth_user_can_update_resource ('map'::public.resource_type, id) and
      public.maps__owner_id_matches_stored (id, owner_id) and
      (
        owner_id = any (
          array(
            select
              public.util__get_workspace_members (maps.workspace_id) as util__get_workspace_members
          )
        )
      )
    )
...
create policy "Users with editor app role can insert maps" on "public"."maps" as permissive for insert to authenticated
with
  check (

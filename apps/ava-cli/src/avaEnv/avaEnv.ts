 ...
 * `--staging` or `--prod` run now needs. {@link requireEnv} is how that
 ...
 * every `requireEnv` call afterwards needs to name the same file. It starts at
 * `local` so a unit test that never loads an env file reports
 ...
 *
 * The env file has to be chosen before any command action runs, and Acclimate
 * hands parsed options to actions rather than to the entry point, so argv is
 * read directly here. `--staging` and `--prod` are still declared as Acclimate
 * global options on the root CLI, which is what makes them legal on every
 * command and documented in every help screen.
 *
 ...
 *
 * `override: true` makes the file authoritative over variables already in the
 * ambient shell. That is not the merging this module exists to prevent: only
 * one file is ever read, so there is no second file for its values to win
 * against.
 *
 * @param options.target Which deployment's file to load.
 * @param options.envFilePath Overrides the resolved path. For tests.
 */
export function loadAvaEnv(
  ...
  if (result.error !== undefined) {
    throw new Error(
      `Failed to load ${fileName}. Run this command from the repo root so we ` +
        "can load the environment variables.",
    );
  }
...
export function getLoadedAvaEnvTarget(): AvaEnvTarget {
  return _loadedTarget;
}

/** The env file this invocation loaded. */
export function getLoadedAvaEnvFile(): string {
...
export function requireEnv(name: string): string {
  ...
  if (!value) {
    throw new Error(`${name} is not set in ${getLoadedAvaEnvFile()}`);
  }
  return value;
}

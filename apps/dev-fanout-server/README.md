...
1. Make sure you've built ava-cli locally with `pnpm build:ava-cli`
...
```bash
docker build -f apps/dev-fanout-server/Dockerfile \
...
```bash
fly deploy --config apps/dev-fanout-server/fly.production.toml
```

...
test.describe("dashboard chat → P-block", () => {
  test("typing a chart request in chat appends a DataViz block to the dashboard", async ({
    ...
    // And the new DataViz block should be on the canvas. We assert the
    // persisted NL prompt is visible inside the Puck-rendered tree.
    await expect(
      page.getByText("California cases by region", { exact: true }).first(),
    ).toBeVisible({ timeout: MEDIUM_WAIT });

...
export async function deleteRetainedOriginalFromCloud(
  ...
  if (!localDataset?.sourceBytes) {
    throw new Error(
      "The original file for this dataset is not stored on this device, so " +
        "removing it from online storage would delete the only copy. Make " +
        "the dataset offline-only from the device that holds the original.",
    );
  }

 ...
 */
export function makeDatasetOriginalFileStoragePathFromIds(options: {
  ...
  if (!VALID_EXTENSION_PATTERN.test(normalizedExtension)) {
    throw new Error(
      `Invalid file extension "${fileExtension}" for dataset original file. The extension must be 1 to 10 alphanumeric characters to be reachable under storage access policies.`,
    );
  }

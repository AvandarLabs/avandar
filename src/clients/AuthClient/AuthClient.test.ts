...
function createFakeAuthProvider(
  session: PlatformSession | undefined,
): AuthProvider {
  const listeners = new Set<
    (authSession: PlatformSession | undefined) => void
  >();
  let current = session;
  return {
    getSession: vi.fn(async () => {
    ...
    signIn: vi.fn(async (credentials) => {
      ...
      const authSession: PlatformSession = {
        userId: "user-1",
...
describe("AuthClient (desktop polyfill)", () => {
  ...
  it("getCurrentSession returns undefined when keychain is empty", async () => {
    installPlatformImpls(createFakeAuthProvider(undefined));
  ...
  it("getCurrentSession returns a Supabase-shaped session when keychain has one", async () => {
    ...
    installPlatformImpls(createFakeAuthProvider(platformSession));
    const session = await AuthClient.getCurrentSession();
  ...
  it("signIn routes through the keychain-backed AuthProvider", async () => {
    const provider = createFakeAuthProvider(undefined);

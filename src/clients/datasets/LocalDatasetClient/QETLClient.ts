 ...
 */
const QETLClientFactory = createModuleFactory<IQETLClient>("QETLClient", {
  childBuilder: (module) => {
    ...
    const MemoryCube = {
       ...
       * data management operations. Just straight up loads a parquet blob into
       * the memory cube.
       * @param datasetId The id of the dataset to load.
       * @param parquetBlob The parquet blob to load.
       * @param columnReplacements An optional record of column replacements to
       * adjust the schema when loading the data into memory.
       * @returns
       */
      loadData: async ({
        datasetId,
    ...
    const DiceManager = {
      determineMissingDice: async ({
        ...
        const depsNotInMemory = difference(queryDependencies, depsInMemoryCube);
    ...
    const OptimizationEngine = {
       ...
       * simply not an option to fetch the entire dataset. We want to construct
       * queries or API calls specific to what we need.
       *
    ...
    const FilteringEngine = {
      loadFacts: async ({
        ...
        // first, we figure out which facts are not in our local storage cache,
        ...
        const factsToCache = facts.filter((fact) => {
          return !factsInLocalStorage.some(propEq("datasetId", fact.datasetId));
        });
...
type IWorkspaceQETLClient = Module<
  "WorkspaceQETLClient",
  EmptyObject,
  {
    runQuery: <RowObject extends UnknownRow = UnknownRow>(params: {

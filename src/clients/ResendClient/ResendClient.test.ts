...
async function _loadResendClient() {
  vi.resetModules();
  return (await import("@/clients/ResendClient/ResendClient")).ResendClient;
}

describe("frontend ResendClient", () => {
  beforeEach(() => {
  ...
  it("rejects the legacy key for sending operations", async () => {
    delete process.env.RESEND_SENDING_API_KEY;
    const ResendClient = await _loadResendClient();
  ...
  it("rejects the legacy key for resource operations", async () => {
    delete process.env.RESEND_FULL_ACCESS_API_KEY;
    const ResendClient = await _loadResendClient();

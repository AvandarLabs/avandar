import { isSameLineRun, lineSpanOf } from "./assembleLabels";
import { normalizeCellValue } from "./normalizeCellValue";
import type { BBox, PdfValueUnit, TextItem } from "./types";
  ...
  scanOrder.forEach((head, headIndex) => {
    ...
    for (let next = headIndex + 1; next < scanOrder.length; next += 1) {
      ...
      if (
        ...
        !isSameLineRun(span, lineSpanOf(candidate))
      ) {
        break;
      }

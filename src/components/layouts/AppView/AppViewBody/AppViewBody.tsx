...
import css from "@/components/layouts/AppView/AppViewBody/AppViewBody.module.css";
...
export function AppViewBody({
  ...
  return (
    <div className={clsx(css.appViewBody, className)}>
      <div className={css.appViewBodyScroller}>
        <div
          className={clsx(
            css.appViewBodyContent,
            noPadding && css.appViewBodyContentNoPadding,
          )}
          style={
        ...
        {rail !== undefined && !isRailAColumn ? (
          <div className={css.appViewBodyStackedRail}>{rail}</div>
        ) : null}
      </div>
      {hasSideRail ? (
        <aside className={css.appViewBodyRail}>{rail}</aside>
      ) : null}
    </div>
  );
}

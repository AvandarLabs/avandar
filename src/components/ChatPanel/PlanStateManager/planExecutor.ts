...
export async function executePlanStep(args: {
  ...
  const { planId, step, dispatch, nodeById } = args;
...
async function _executeSandboxStep(args: {
  ...
  const { planId, step, runtime, dispatch, nodeById } = args;
  dispatch.markStepRunning(step.id);
...
export async function executePlan(args: {
  ...
  // Resolve view names by upstream id so python/r steps can pull
  // their input parquet bytes out of DuckDB without re-scanning the
  // plan on every step. We rebuild this each iteration since
  // `markStepSucceeded` will have populated `viewName` on previous
  // nodes — but for the first iteration we use the existing plan
  // shape (passed in via `nodes`).

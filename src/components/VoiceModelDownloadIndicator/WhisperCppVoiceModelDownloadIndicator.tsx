...
/**
 * Download progress for the parallel whisper.cpp WASM voice pipeline.
 * Offset below the transformers indicator when both are active.
 */
export function WhisperCppVoiceModelDownloadIndicator(): JSX.Element | null {
  const { t } = useLingui();
  ...
  const message = t`Downloading ${model.displayName} for whisper.cpp WASM`;
  ...
  return (
    <div
      className={css.rootWhisperCpp}
      role="status"
      aria-live="polite"
      aria-label={message}
    >
      <div className={css.titleRow}>
        <p className={css.title}>{model.displayName} (WASM)</p>

...
/** Fingerprints the layer inputs that can change a derived legend. */
export function buildLayerLegendFingerprint(layer: MapLayer.T): string {
...
function _hasApplicableUpdate(
  ...
  return mapConfig.layers.some((layer) => {
    ...
    return (
      update !== undefined &&
      update.layerFingerprint === buildLayerLegendFingerprint(layer) &&
      !_isLegendEqual(layer, update)
    );
  });
}

/** Persists fresh derived legend output through one immutable config update. */
export function usePersistedLayerLegends(options: Options): void {

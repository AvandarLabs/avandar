...
/** Natural-break cut points and whether the input values were sampled. */
...
function _segmentVariance(
  ...
  return squaredSum - (sum * sum) / segmentLength;
}
...
export function makeJenksBreaksFromValues(options: {
  values: readonly number[];
  classCount: number;
}): JenksBreakResult {
  ...
  const classCount = Math.min(options.classCount, distinctValueCount);
  return {

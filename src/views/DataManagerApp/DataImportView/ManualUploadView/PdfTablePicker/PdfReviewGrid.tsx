 ...
 *
 * See `PdfCellFlag` in `src/workers/pdfSniff/types.ts`. Everything that walks
 ...
 */
function isRegionFlag(flag: PdfCellFlag): boolean {
...
export function PdfReviewGrid({
  ...
  const cellFlags = table.flags.filter((flag) => {
    return !isRegionFlag(flag);
  });
  const regionFlags = table.flags.filter(isRegionFlag);
  ...
  const editCell = (
    ...
    onTableChange({
      ...
      flags: table.flags.filter((flag) => {
        return isRegionFlag(flag) || flag.rowIndex !== rowIndex;
      }),
    });
  };
    ...
    <Stack gap="sm">
      {flaggedRowCount > 0 && (
        <Alert variant="light" color="yellow" icon={<IconAlertTriangle />}>
          <Text size="sm">
            {flaggedRowCount} of {dataRows.length} rows{" "}
            {flaggedRowCount === 1 ? "needs" : "need"} review. We matched these
            values to their labels by position, and these were close calls.
            Check them against the page.
          </Text>
        </Alert>
      )}
       ...
       */}
      {cellFlags.map((flag, index) => {
        return (
          <Text
            key={`${flag.rowIndex}-${flag.columnIndex}-${index}`}
            size="xs"
            c="dimmed"
          >
            {`Row ${flag.rowIndex + 1}: ${flag.detail}`}
          </Text>
        );
      })}
    </Stack>

/**
 * Unit tests for the Data Explorer drawer's height math: the clamp applied to a
 * requested height and the keyboard resize steps on the resize separator.
 *
 * `availableHeight` is the height of the whole region the chart and the drawer
 * split between them, so 900 below stands for that combined region.
 */
...
describe("DrawerHeight.clamp", () => {
  ...
  it("caps the height so the chart keeps at least 40% of the region", () => {
...
describe("DrawerHeight.resolveMaxHeight", () => {
  it("reports the share of the region the drawer may occupy", () => {
    expect(DrawerHeight.resolveMaxHeight(900)).toBe(540);
  });

  it("never reports a cap below the minimum height", () => {
    expect(DrawerHeight.resolveMaxHeight(200)).toBe(DrawerHeight.MIN_HEIGHT);
  });

  it("reports no cap for an unmeasured region", () => {
    expect(DrawerHeight.resolveMaxHeight(0)).toBeUndefined();
  });
});

describe("DrawerHeight.resolveHeightForKey", () => {
  const availableHeight = 900;

  it("grows the drawer on ArrowUp because the drawer extends upward", () => {
    expect(
      DrawerHeight.resolveHeightForKey({
        key: "ArrowUp",

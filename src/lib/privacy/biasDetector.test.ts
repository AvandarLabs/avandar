...
describe("detectBias", () => {
  ...
  it("does not flag affirming statements about diversity", () => {
    expect(detectBias("women are diverse in our dataset").hits).toHaveLength(
      0,
    );
  });
  ...
  it("flags loaded \"why are X poor\" framing", () => {
    ...
    expect(result.hits.some((h) => h.category === "loaded_framing")).toBe(true);
  });
  ...
  it("flags \"normal household\" framing", () => {
    const result = detectBias("compare normal households to single-parent ones");
  ...
  it("deduplicates by label", () => {
    const text =
      "women are always X. women are never Y. women are usually Z.";
    ...
    // only fires once.
    const labels = new Set(generalizationHits.map((h) => h.label));

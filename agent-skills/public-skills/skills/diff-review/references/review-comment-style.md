...
How to write the prose of a code-review comment **as the reviewer**: the
comments you leave on someone else's diff. Rules about comments *inside the
code* belong to the repository's own documentation.

This is not the file for replying to a review. When you are the engineer
answering a reviewer's comment, follow
[`reply-style.md`](reply-style.md) instead. The two voices are different: a
reviewer opens a topic, an engineer closes one.

A personal override may exist at `~/.diff-review/review-comment-style.md`.
When it does, read it and let it win on any conflict with this file.

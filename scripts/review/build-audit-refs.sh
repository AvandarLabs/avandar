...
#                           paths advanced to develop. `git diff review/base
...
# Applying every tier's paths at once must reproduce develop's tree exactly.
FULL=$(build_tree "${ALL[@]}" "${T6[@]}")
if [ "$FULL" = "$(git rev-parse "$TIP^{tree}")" ]; then
  echo "  ✔ tiers reproduce ${TIP}'s tree exactly — no path escapes review"
else
  echo "  ✘ COVERAGE GAP:"; git diff --name-only "$FULL" "$TIP" | sed 's/^/      /'
  exit 1
fi
